...
#define INVALID_COL(lp, colnr) ((colnr > lp->columns+1) || (colnr < 1))
char * __WINAPI get_col_name(lprec *lp, int colnr)
{
  if INVALID_COL(lp, colnr) {
...
/* #define v51mode */ /* Enable this for v5.1 operation mode */
  ...
  if(lp->spx_trace) {
    const char *psz_primal = primal ? "PRIMAL" : "DUAL";
    if(minitNow)
    ...
    if(minitNow) {
      if(!lp->is_lower[varin])
        report(lp, DETAILED,
        "performiteration: %s variable %d changed to its lower bound at iter %.0f (from %g)\n",
	       psz_primal, varin, (double) get_total_iter(lp), enteringUB);
      else
        report(lp, DETAILED,
        "performiteration: %s variable %d changed to its upper bound at iter %.0f (to %g)\n",
	       psz_primal, varin, (double) get_total_iter(lp), enteringUB);
    }
    else 
      report(lp, NORMAL,
	     "performiteration: %s variable %d entered basis at iter %.0f at " RESULTVALUEMASK "\n",
	     psz_primal, varin, (double) get_total_iter(lp), lp->rhs[rownr]);
    if(!primal) {
