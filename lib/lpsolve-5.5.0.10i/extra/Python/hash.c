...
hashtable_t *create_hash_table(int size, int base)
{
  ...
  ht = (hashtable_t *) calloc(1 , sizeof(*ht));
  ht->table = (hashelem_t **) calloc(size, sizeof(*(ht->table)));
  ht->size = size;
  ht->base = base;
  ht->count = base-1;
		    ...
		    hashtable_t *ht)
{
  ...
  if((hp = findhash(name, ht)) == NULL) {
    ...
    hp = (hashelem_t *) calloc(1, sizeof(*hp));
    hp->name = strdup(name);
    hp->index = index;
    ht->count++;
    if(list != NULL)
