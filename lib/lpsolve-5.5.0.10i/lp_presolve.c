...
STATIC 
int presolve_coldominance01(presolverec *psdata, NATURAL_t *nConRemoved, 
			NATURAL_t *nVarsFixed, NATURAL_t *nSum)
   ...
   that are dominated via set coverage or unit knapsack constraints */
{
  lprec     *lp = psdata->lp;
  MATrec    *mat = lp->matA;
  NATURAL_t i, ib, ie, jx, item, item2,
            n = lp->int_vars, iVarFixed = 0, nrows = lp->rows,
            *coldel = NULL;
  int       jb, jj, ii, status = RUNNING;
  REAL      rhsval,
  ...
  for(i = firstActiveLink(psdata->rows->varmap); i != 0; 
  ...
  /* A column dominates another binary variable column with the
     following criteria:

      1) The relative matrix non-zero entries are identical

      2) The relative objective coefficient is worse than the other;
         if the OF coefficients are identical, we can delete an
         arbitrary variable */
  n = 0;
  for(i = firstActiveLink(psdata->cols->varmap); i != 0; 
  ...
  for(ib = 0; ib < n; ib++) {
    ...
    /* Loop over all other columns to see if they have equal
    ...
    for(ie = ib+1; ie < n; ie++) {
      ...
      /* Insist on identical column lengths (sort is decending in
	 column lengths) */
      ...
      /* Loop over every column member to confirm that the candidate
         is identical in every row; we also compute the minimal set
         order */
      rhsval = lp->infinite;
      item = 0;
      item2 = 0;
    ...
    if(coldel[0] > 1) {
      qsortex(colobj+1, coldel[0], 0, sizeof(*colobj), FALSE, compareREAL, 
	      coldel+1, sizeof(*coldel));
      jb = (NATURAL_t) (rhsval+lp->epsvalue);
      for(jb++; jb <= coldel[0]; jb++) {
        jx = coldel[jb];
        if(!presolve_colfix(psdata, jx, lp->orig_lowbo[nrows+jx], 
			    TRUE, &iVarFixed)) {
	  status = presolve_setstatus(psdata, INFEASIBLE);
	  goto Finish;
        }
